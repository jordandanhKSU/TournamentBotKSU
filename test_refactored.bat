...
echo ========================================================
echo             Testing Refactored TournamentBot
echo ========================================================
echo.
...
REM Run the test_imports.py script to verify imports are working
python Scripts\TournamentBot\test_imports.py

if %ERRORLEVEL% NEQ 0 (
    echo.
    echo Import tests failed with error code %ERRORLEVEL%.
    goto :error
)
...
echo    pip install -r requirements.txt
echo.
echo 2. If imports are failing:
echo    - Check that your PYTHONPATH includes the project directory
echo    - Look for missing packages in the error messages
echo    - Verify that all __init__.py files are in place
echo.
